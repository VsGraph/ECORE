...
def generate_sample_rate_vector(Dataset_choice, num_stage_predicate):
    if Dataset_choice == 'VG':
        ...
        # Approximate frequency counts for 30+1 OI_V6 predicates (index 0 = background)
        predicate_new_order_count = [5000000, 800000, 600000, 500000, 300000, 200000, 150000,
                                     100000, 80000, 60000, 50000, 40000, 35000, 30000, 28000,
                                     25000, 22000, 18000, 15000, 12000, 10000, 8000, 6000,
                                     5000, 4000, 3500, 3000, 2500, 2000, 1500, 1000]
        assert len(predicate_new_order_count) == 31
    else:
