...
class MotifsLike_GCL(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.aum_lambda = config.MODEL.ROI_RELATION_HEAD.AUM.LAMBDA / (2 * self.hidden_dim)
    ...
    def generate_aum_log_var_branches(self, num_cls):
        ...
        Weights are zero-initialized; biases are set to a dimension-scaled
        prior log(d/4) so that exp(s) starts at d/4, reflecting a broad
        initial uncertainty proportional to feature dimensionality.
        """
        _logvar_prior = np.log(self.pooling_dim / 4.0)

        def _make_branch(out_dim):
            fc = nn.Linear(self.pooling_dim, out_dim)
            nn.init.zeros_(fc.weight)
            nn.init.constant_(fc.bias, _logvar_prior)
            return fc
