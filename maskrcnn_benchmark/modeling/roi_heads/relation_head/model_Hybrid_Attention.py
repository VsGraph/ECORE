...
# ===========================================================================
# AFEM: Attention-guided Feature Enhancement Module
# Core component of the Multi-modal Evidence-aware Fusion (MEF) module.
# Enhances union visual features using semantic guidance from GloVe embeddings.
# ===========================================================================
class AFEM(nn.Module):
    def __init__(self, union_dim, sem_dim, hidden_dim,
                 n_head, d_k, d_v, d_inner, dropout=0.1):
        super(AFEM, self).__init__()
        # project semantic pair feature (glove_s + glove_o) to hidden_dim
        self.sem_proj = nn.Linear(sem_dim, hidden_dim)
        # project union visual feature to hidden_dim
        self.vis_proj = nn.Linear(union_dim, hidden_dim)
        # multi-head self-attention over projected semantic features
        self.sa_encoder = Self_Attention_Encoder(
            n_head, d_k, d_v, hidden_dim, d_inner, dropout)
        # cross-modal fusion: LayerNorm(SA(sem) + vis)
        self.norm_fusion = nn.LayerNorm(hidden_dim)
        # position-wise feed-forward with residual
        self.ffn = nn.Sequential(
            nn.Linear(hidden_dim, d_inner),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
            nn.Linear(d_inner, hidden_dim),
            nn.Dropout(dropout),
        )
        self.norm_out = nn.LayerNorm(hidden_dim)

        nn.init.xavier_uniform_(self.sem_proj.weight)
        nn.init.xavier_uniform_(self.vis_proj.weight)

    def forward(self, union_feat, sem_pair_feat, num_rels):
        """
        Args:
            union_feat    : [N_rel, union_dim]   union visual features
            sem_pair_feat : [N_rel, sem_dim]     semantic pair feature (glove_s + glove_o)
            num_rels      : list[int]            number of relations per image
        Returns:
            enhanced_union: [N_rel, hidden_dim]  semantically enhanced union features
        """
        sem_proj = self.sem_proj(sem_pair_feat)  # [N, D]
        vis_proj = self.vis_proj(union_feat)      # [N, D]

        # multi-head self-attention on semantic projections
        sem_attn = self.sa_encoder(sem_proj, num_rels)  # [N, D]

        # cross-modal fusion: LayerNorm(SA(sem) + vis)
        fused = self.norm_fusion(sem_attn + vis_proj)

        # residual FFN: LayerNorm(FFN(fused) + fused)
        enhanced_union = self.norm_out(self.ffn(fused) + fused)
        return enhanced_union
